...
export const metadata: Metadata = {
  title: "Tinjauan Gizi Ibu — Nutritionist Workspace",
...
export default async function NutritionistAssessmentDetailPage({
  params,
}: {
  ...
  if (manifest.level2Reviewer !== "NUTRITIONIST" && session.user.role !== "ADMIN") {
    redirect("/nutritionist/review");
  }
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl mx-auto space-y-6 pb-16">
      ...
      <nav className="flex items-center gap-2 text-sm text-muted-foreground">
        <Link href="/nutritionist/review" className="hover:text-primary transition-colors">
          Antrean Review
        </Link>
        <span>/</span>
        ...
        <div className="space-y-6">
          ...
          {canReview && (
            <div className="bg-card border border-border rounded-2xl p-6 space-y-4 shadow-sm">
              ...
              <ClinicalDecisionForm assessmentId={assessment.id} reviewerPath="nutritionist" />
            </div>
          )}

...
export default async function NutritionistLayout({
  ...
  const userName = session.user.name ?? "Ahli Gizi";
    ...
    <div className="min-h-screen flex bg-muted/30">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-card border-r border-border">
        ...
        <div className="px-6 py-5 border-b border-border">
          <Link href="/nutritionist" className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center">
              <span className="text-white font-bold text-sm">N</span>
            </div>
            <span className="font-bold text-foreground">{userName}</span>
          </Link>
          <p className="text-xs text-muted-foreground mt-2">Portal Ahli Gizi</p>
        </div>
